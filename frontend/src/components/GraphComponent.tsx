...
export default function GraphComponent() {
  ...
  const isValidPosition = (x, y, parentX, parentY, existingPositions, minDistanceFromParent, minDistanceBetweenNodes) => {
  ...
  const layoutChildNodes = (nodes, edges, parentNode, sectionType) => {
    ...
    childNodes.forEach((node, index) => {
      ...
      const effectiveMinDistance = randomConfig.minDistance + 
      ...
      while (!validPosition && attempts < 20) {
        ...
        const baseAngle = randomConfig.angleStart + (angleRange * (index / Math.max(childNodes.length, 1)));
        
        // Add some randomness to the angle
        const angle = baseAngle + randomInRange(-0.15, 0.15);
        
        // Random distance from parent within boundaries
        const distance = randomInRange(randomConfig.minDistance, randomConfig.maxDistance);
      ...
      if (!validPosition && positionedNodes.length > 0) {
        ...
        for (let i = 0; i < 5; i++) { // Apply force iterations
          let forceX = 0;
          let forceY = 0;
          ...
          for (const pos of positionedNodes) {
            ...
            if (dist < randomConfig.minNodeDistance && dist > 0) {
              ...
              const fx = (posX - pos.x) / dist * (randomConfig.minNodeDistance - dist) * 0.2;
              const fy = (posY - pos.y) / dist * (randomConfig.minNodeDistance - dist) * 0.2;
              forceX += fx;
              forceY += fy;
            }
          }
  ...
  const shuffleLayout = () => {
    ...
    // Get parent nodes
    const parentNodes = newNodes.filter(node => 
